import { AsyncAction, CancelledError } from '@neo/commons/utils/hex/async_action';
...
describe('AsyncAction', () => {
  ...
  test('Timeout Error', async () => {
    // Build the async action to not resolve until after 1 ms
    ...
    // Set the timeout limit on the action to 0 ms
    action.timeoutIn(0);

    // Return the action execution and expect it to fail.  Catch the failure and confirm its a timeout error
